...
#### 8.4. 将来的な管理アクション (このフェーズでは表示のみ -> 削除機能追加済み)

各コネクションアイテムの横には、以下のようなアクションボタン/アイコンを配置することを想定しています。

*   **編集ボタン:** (未実装) クリックすると、`CreateConnectionDialog` をそのコネクションの既存データでプリフィルして開きます。
...
#### 8.6 表示用メイン写真の選択 (Selecting Main Display Photo) (新規・実装済み)
...
- **既存コネクションの削除 (人物編集ダイアログ内) (実装済み - 確認ダイアログを含む)**
...
既存コネクションの表示・削除機能が追加されることで、人物情報の全体像がより把握しやすくなります。
